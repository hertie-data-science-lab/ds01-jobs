...
ALLOWED_REGISTRIES = ("nvcr.io/nvidia/",)

BLOCKED_ENV_VARS = {"LD_PRELOAD", "LD_LIBRARY_PATH"}

BUILD_TIMEOUT_SECONDS = (
    900  # 15 minutes — enforced in Phase 14 runner, defined here as constant
)
...
def scan_dockerfile(content: str) -> list[dict]:
        ...
        return [
            {
                "line": 0,
                "directive": "PARSE",
                "reason": f"Invalid Dockerfile syntax: {e}",
            }
        ]
    ...
    for cmd in commands:
        ...
        if directive == "from":
            ...
            if not any(
                image.startswith(r) for r in ALLOWED_REGISTRIES
            ) and not is_docker_hub_official(image):
                violations.append(
                    {
                        "line": cmd.start_line,
                        "directive": "FROM",
