...
- [ ] **JOB-03**: Structured 422 response with machine-parseable field-level errors on validation failure
- [ ] **JOB-04**: GitHub URL validation (only https://github.com/owner/repo accepted — SSRF prevention)
- [ ] **JOB-05**: Optional inline dockerfile_content field for pre-scan at submission time

### Dockerfile Scanning

- [ ] **SCAN-01**: All FROM directives scanned across all stages (multi-stage bypass prevention)
- [ ] **SCAN-02**: Only approved base registries allowed: nvcr.io/nvidia/* and Docker Hub official images
- [ ] **SCAN-03**: ENV LD_PRELOAD and LD_LIBRARY_PATH blocked outright (CVE-2025-23266 mitigation)
- [ ] **SCAN-04**: USER root produces warning (non-blocking) — cgroup constraints apply regardless
- [ ] **SCAN-05**: Scan violations return structured error with line number, directive, and reason
...
- [ ] **RUN-01**: Runner polls SQLite for queued jobs and executes them (separate systemd service)
- [ ] **RUN-02**: Job lifecycle: clone repo → docker build (no --gpus) → docker run (via wrapper) → capture output
- [ ] **RUN-03**: Status transitions committed to SQLite before each subprocess starts (queued → cloning → building → running → succeeded/failed)
- [ ] **RUN-04**: stdout/stderr written to log files per phase (clone.log, build.log, run.log), not PIPE
- [ ] **RUN-05**: GPU slot management: check real GPU availability (nvidia-smi / allocator state) before dispatch, not internal SUM query — respects GPUs used by interactive containers
- [ ] **RUN-06**: Build timeout (15 min) and configurable job timeout enforced via process group kill
- [ ] **RUN-07**: Startup recovery: detect orphaned running/building/cloning jobs on restart, mark as failed
- [ ] **RUN-08**: Graceful shutdown: SIGTERM handler drains running jobs before stopping
- [ ] **RUN-09**: Job cancellation via POST /api/v1/jobs/{id}/cancel — kills process group
- [ ] **RUN-10**: Docker build cache cleanup after each job to prevent disk exhaustion
- [ ] **RUN-11**: All docker commands go through /usr/local/bin/docker (wrapper) for cgroup/GPU enforcement

### Status & Results

- [ ] **STAT-01**: User can poll job status via GET /api/v1/jobs/{id} (status, phase timestamps, error message)
...
- [ ] **STAT-04**: User can check remaining quota via GET /api/v1/users/me/quota
